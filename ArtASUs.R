...
ggplot(data.scores, aes(x = NMDS1, y = NMDS2))+ 
  ...
  annotate("text", x = -0.6, y = 1.2, label = paste0("stress: ", format(nmds$stress, digits = 4)), hjust = 0, size = 10)+
  annotate("text", x = 0.42, y = 1.2, label =(expression(paste("R = 0.9790"))), size = 10)+
  annotate("text", x = 1.1, y = 1.2, label =(expression(paste(,italic("P"),"= 0.0073"))), size = 10)
...
IVICG<- IVI[IVI$Substrate=="CG",]

ggplot() + 
  geom_bar(data=IVIASUs, aes(x = Species, y=-IVI, fill=Substrate), stat="identity")+
  geom_bar(data=IVICG, aes(x = Species, y=IVI, fill=Substrate), stat="identity")+
